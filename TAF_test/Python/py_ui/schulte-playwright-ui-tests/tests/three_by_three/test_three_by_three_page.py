...
@allure.story("3x3 Grid")
@pytest.mark.usefixtures("setup")
class TestThreeByThreePage:

    @allure.title("Navigate to 3x3 grid")
    def test_navigate_to_three_by_three(self, browser):
        menu_service = MenuNavigationService(browser)
        menu_service.navigate_to_three_by_three()
        page_obj = ThreeByThreePage(browser)
        assert page_obj.is_displayed(), "3x3 board is not displayed"
