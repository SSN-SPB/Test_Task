...
class TestNineByNinePage:
    ...
    def test_table_has_correct_cell_count(self, browser):
        page_obj = MenuNavigationService.return_selected_grid(browser, 9)
        assert (
            page_obj.has_correct_number_of_cells()
        ), "Board does not have 81 cells"
    ...
    def test_cells_contain_unique_numbers(self, browser):
        ...
        ), "Board does not contain 81 unique numbers"
    ...
    def test_description_text(self, browser):
        ...
        assert (
            "1 to 81" in desc
        ), f"Description '{desc}' does not mention 1 to 81"
    ...
    def test_click_correct_number_advances(self, browser):
        MenuNavigationService.return_selected_grid(browser, 9)
