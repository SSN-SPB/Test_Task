...
--570. Managers with at Least 5 Direct Reports Passed 4-Jul-2026
--Table: Employee
--
--+-------------+---------+
--| Column Name | Type    |
...
SELECT E1.name FROM EMPLOYEE E1 WHERE E1.ID IN
    (
    SELECT M1.managerID FROM
        (
        SELECT E.managerID, COUNT(E.managerID) AS NUMBER FROM EMPLOYEE E GROUP BY E.managerID
        ) M1
        WHERE M1.NUMBER > 4
    )
...
608 Tree Node https://leetcode.com/problems/tree-node/submissions/2040961463/
...
Solution
SELECT F.product_id, F.first_year, S2.quantity, S2.price  FROM
    (SELECT S.product_id, MIN(S.year) as first_year FROM SALES AS S GROUP BY S.product_id) AS F
    LEFT JOIN SALES AS S2 ON F.product_id = S2.product_id AND F.first_year = S2.year
...
SELECT DISTINCT(P.product_id), product_name FROM Product P
    ...
    WHERE S.sale_date >= '2019-01-01' AND S.sale_date < '2019-04-01' AND S.product_id NOT IN
    (
        SELECT SPS.product_id FROM Sales SPS WHERE SPS.sale_date < '2019-01-01' OR SPS.sale_date >= '2019-04-01'
    )
...
SELECT S1.day, COUNT(S1.day) as active_users FROM
    (
    SELECT A.activity_date AS day, A.user_id as active_users
    FROM Activity as A
