...
def test_check_not_collapsed_elements_submenu(introduction):
    ...
    checked_text_not_displayed_after_collapse_menu = WebDriverWait(driver, 10).\
        until(ec.invisibility_of_element_located((By.XPATH, '//li[@id="item-0"]/span')))
...
def test_elements_text_box(introduction):
    ...
    assert text_box_text.is_displayed(), 'Check2 the Left Text-Box button is displayed by default'
    # elements_tabs = driver.find_elements(By.XPATH, '//div[@class="header-text"]/span')
    print('Find text body element if subtabls of elements is not displayed')
    elements_textbox_not_selected_indicator = driver.find_element(By.XPATH, '//div[@class="container playgound-body"]/div[2]/div[2]')
    ...
    assert elements_textbox_not_selected == 'Please select an item from left to start practice.'
    print('Check the changed text before after clicking Left Text-Box button')
    text_box_text.click()
    full_name_textbox_selected = driver.find_element(By.XPATH,'//*[@id="userName-label"]')
