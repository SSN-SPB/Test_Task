...
def main():
    ...
    #login_to_site()
    login_to_site(username=args.get('logon'),
                  url=args.get('website') ,
                  userpassword =args.get('user_password'))
...
def login_to_site(url = 'https://wirecard-sg.atlassian.net/browse/WDASLANDWH-2558',
    ...
    driver = webdriver.Chrome(executable_path=r"C:\DiskD\trainings\addedToPath\chromedriver.exe",options=options);
    # define weblink
    # weblink = url
    driver.get(str(url))
