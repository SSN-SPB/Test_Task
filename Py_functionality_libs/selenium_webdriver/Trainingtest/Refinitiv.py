...
def login_to_site(url = 'http://localhost:8080',
    ...
    driver = webdriver.\
             Chrome(options=options,\
                    executable_path\
                    =r'C:\DiskD\trainings\addedToPath\chromedriver.exe')
    ####### For installing the zip with chromedriver each time
    ####### driver = webdriver.Chrome(ChromeDriverManager().install())
    driver.get(url)
    login_name = driver.find_element_by_css_selector\
                 ('input#AAA-AS-SI1-SE003')
    login_name.clear()
    login_name.send_keys(username)

    login_pass = driver.find_element_by_css_selector\
                 ('input#AAA-AS-SI1-SE006')
    login_pass.clear()
    login_pass.send_keys(userpassword)
    confirm_but = driver.find_element_by_css_selector\
    ('div#AAA-AS-SI1-SE014')
    confirm_but.click()
...
def cont():    
    ...
    confirm_elem = driver.find_element_by_css_selector('button.btn.btn-primary'
                                                       '.js-cookies-accept-all')
    confirm_elem.click()

    # switch to register page
    # <span class="action-text welcome-header">
    login_elem = driver.find_element_by_css_selector\
                 ('span.action-text.welcome-header')
    login_elem.click()
