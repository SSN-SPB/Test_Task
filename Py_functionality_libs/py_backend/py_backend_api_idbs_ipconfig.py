...
def post_iprange(
    ...
    print(
        "Adding range return code is {} and description is '{}'".format(
            post_iprange, json.loads(post_iprange.text)
        )
    )
...
def add_new_range(get_endpoint, bear_token, iprange, ipdescription):
    ...
    if modified_ip != iprange:
        print("Ip you want to add {} is modified to {}.".format(iprange, modified_ip))
    if not (modified_ip in list_of_cidrs):
        post_iprange(get_endpoint, bear_token, modified_ip, ipdescription)
    else:
        print(
            "Your {} is treated as {}, but it is in list already".format(
                iprange, modified_ip
            )
        )
