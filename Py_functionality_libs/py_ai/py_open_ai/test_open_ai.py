...
def test_generate_text_length(ai_model):
    ...
    assert 10 <= len(response) <= 500, "Generated text length is not within expected range."
...
def test_generate_text_safety(ai_model):
    ...
    assert not any(word in response.lower() for word in toxic_words), "Generated text contains harmful content!"
