...
def get_kafka_messages(server, topic, min_time, expected_event_type=None, ssl_cafile=None, ssl_keyfile=None,
                       ssl_certfile=None):
    ...
    for message in test_consumer:
        try:
            message_timestamp = set_datetime_object(json.loads(message.value)["timestamp"])
        except (ValueError, KeyError):
            print("Oops!  That was no valid 'timestamp' key in message: {}".format(message.value))
            continue
        ...
        if len(message_list) > 0 and test_consumer.position(tp) >= count and check_expected_event_type(
...
def check_expected_event_type(expected_event_type, message):
    if (expected_event_type is not None and json.loads(message.value)["eventType"] == expected_event_type) \
