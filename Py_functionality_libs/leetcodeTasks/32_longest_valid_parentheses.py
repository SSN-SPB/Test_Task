...
def main():
    ...
    # assert c.longestValidParentheses(tested_string[0]) == tested_string[1]
    for x in range(0, len(tested_strings)):
        print(f" result : {c.longestValidParentheses(tested_strings[x][0])}")
        assert c.longestValidParentheses(tested_strings[x][0]) == tested_strings[x][1]
