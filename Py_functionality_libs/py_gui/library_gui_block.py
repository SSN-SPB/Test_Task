...
def popup_label(root, label_text, label_font=font_Arial_12, place_x=10, place_y=10):
...
def popup_input_text(
    ...
    textname = tkinter.Entry(popup_name, width=text_width, bd=3, font=label_font)
...
def popup_input_text_list(
    root, list_content, place_x, place_y, listbox_height, listbox_width
):
    ...
    textname_list = tkinter.Entry(popup_name, width=text_width, bd=3, font=label_font)
    textname.place(x=place_x, y=place_y)
    return textname_list


def popup_input_multiline_text(
    root, height_input, place_x=10, place_y=100, width_of_multitext=30
):
    input_multiline_text = ScrolledText(
        root, height=height_input, width=width_of_multitext
    )
...
def popup_label_grid(root, label_text, row_number, justify_lab):
    lab = tkinter.Label(root, text=label_text, justify=justify_lab).grid(row=row_number)


def popup_input_text_grid(popup_name, row_number, column_number):
    input_text_field_grid = tkinter.Entry(popup_name).grid(
...
def popup_button(
    ...
    function_name,
    button_width=20,
    button_font=font_Arial_12,
    place_x="10",
    place_y="10",
):
    but = tkinter.Button(
        popup_name, text=press_buttom_text, width=button_width, font=button_font
    )
